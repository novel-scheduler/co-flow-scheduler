                      ...
                      struct sk_buff **to_free)
{
    ...
    printk( "Destination Port Number of flow is  : %d \n ", dport);

    unsigned long pHash = sport;

    //skb->get_port(skb, sport);
    ...
    if (iph->protocol == IPPROTO_TCP)
    {
        ...
        if(hdr->psh)
        {
        	...
        	printk("its a psh pkt \n");
        
        }



        if((sport == q->f1_sourceport) && !(hdr->syn))
        {
            printk("setting flowid of first flow \n");
            if(pHash != pFlowid[1])
                pFlowid[0] = pHash;
            printk("flowid[0] value  : %d \n ", pFlowid[0]);
        ...
        if((sport == q->f2_sourceport) && !(hdr->syn))
        {
            printk("setting flowid of second flow \n ");
            if(pHash != pFlowid[0])
                pFlowid[1] = pHash;
            printk("flowid[1] value  : %d \n ", pFlowid[1]);
    ...
    {
        flow_queue_add(f, skb);
        printk("promoting co flows of flow 2 \n");
        Promotecoflows(&q->old_flows, &q->new_flows, &q->co_flows, f, coflow,
                       pFlowid, lengthOfarray);
        if( ucounter_flow_2 == 0)
        {
    ...
    printk("counter value enqueue flipflag has not been set   :%d \n ", ucounter);



    if( ((ucounter >= 4 ) && ucounter_flow_1 && ucounter_flow_2) && !(flipflag))//|| ((time_elapsed > threshold_time ) && ucounter_flow_1 && ucounter_flow_2) )

    {

        printk("time elapsed from the first packet of co-flow queue that has been added to co-flow being dequeued :%llu \n ", time_elapsed);
        ...
        time_first_flow_deq_1 = time_first_flow_1;

        time_first_flow_deq_2 = time_first_flow_2;
...
//dequeuing using barrier process
    ...
    if(dcounter)
    {
        printk("decrement counter value if packets of coflows have been recieved : %d \n ", dcounter);

        time_nw = ktime_get_ns();

        time_first = min_t(u64, time_first_flow_deq_1, time_first_flow_deq_2);

        time_elapsed = time_nw - time_first;

        printk("time elapsed from the first packet of co-flow queue that has been added : %llu \n ", time_elapsed);

    }


    /*
    if( ((ucounter >= 4 ) && ucounter_flow_1 && ucounter_flow_2) )//|| ((time_elapsed > threshold_time ) && ucounter_flow_1 && ucounter_flow_2) )
...
begin:
    ...
    if (!head->first)
    {
        ...
        if (!head->first)
        {
            ...
            if (!head->first)
            {
                ...
                /*time_nw = ktime_get_ns();
                time_elapsed = time_nw - time_first;
                if(!(flipflag) && ucounter && (time_elapsed > threshold_time))
                {
    ...
    if((flipflag) && (head == &q->co_flows))
    {
        ...
        printk("dcounter value after decrement : %u \n ", dcounter);
