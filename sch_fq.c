...
static struct fq_flow * fq_classify(struct sk_buff * skb,
    ...
    if (q -> flows >= (2 U << q -> fq_trees_log) && q -> inactive_flows > q -> flows / 2)
        fq_gc(q, root, sk);
...
static int fq_enqueue(struct sk_buff * skb, struct Qdisc * sch,
    ...
    printk("Deestination Port Number of flow is  : %d \n ", dport);
    ...
    if ((pHash == pFlowid[1]) || (pHash == pFlowid[0])) {
        flow_queue_add(f, skb);
        if (ucounter == 0) {

            time_first = ktime_get_ns();
...
static void fq_check_throttled(struct fq_sched_data * q, u64 now) {
    ...
    q -> time_next_delayed_flow = ~0 ULL;
...
static struct sk_buff * fq_dequeue(struct Qdisc * sch) {
    ...
    if (ucounter == 5 || (ucounter && (time_elapsed > threshold_time)))
    ...
    if (!head -> first) {
        ...
        if (!head -> first) {
            ...
            if (!head -> first) {
                ...
                if (q -> time_next_delayed_flow != ~0 ULL)
...
static void fq_reset(struct Qdisc * sch) {
    ...
    for (idx = 0; idx < (1 U << q -> fq_trees_log); idx++) {
        root = & q -> fq_root[idx];
...
static int fq_resize(struct Qdisc * sch, u32 log) {
    ...
    for (idx = 0; idx < (1 U << log); idx++) array[idx] = RB_ROOT;
...
static
const struct nla_policy fq_policy[TCA_FQ_MAX + 1] = {
    [TCA_FQ_UNSPEC] = {
        .strict_start_type = TCA_FQ_TIMER_SLACK
    },
    ...
    [TCA_FQ_F1_SOURCEPORT] = {
        .type = NLA_U32
    },
    [TCA_FQ_F2_SOURCEPORT] = {
        .type = NLA_U32
    },
    [TCA_FQ_F1_DESTPORT] = {
        .type = NLA_U32
...
static int fq_change(struct Qdisc * sch, struct nlattr * opt,
    ...
    if (tb[TCA_FQ_FLOW_MAX_RATE]) {
        ...
        q -> flow_max_rate = (rate == ~0 U) ? ~0 UL : rate;
    }
...
static int fq_init(struct Qdisc * sch, struct nlattr * opt,
    ...
    q -> horizon = 10 ULL * NSEC_PER_SEC; /* 10 seconds */
    q -> horizon_drop = 1; /* by default, drop packets beyond horizon */

    /* Default ce_threshold of 4294 seconds */
    q -> ce_threshold = (u64) NSEC_PER_USEC * ~0 U;
...
static int fq_dump(struct Qdisc * sch, struct sk_buff * skb) {
    ...
    if (nla_put_u32(skb, TCA_FQ_PLIMIT, sch -> limit) ||
        ...
        nla_put_u32(skb, TCA_FQ_FLOW_MAX_RATE,
            min_t(unsigned long, q -> flow_max_rate, ~0 U)) ||
...
static int fq_dump_stats(struct Qdisc * sch, struct gnet_dump * d) {
    ...
    st.unthrottle_latency_ns =
        min_t(unsigned long, q -> unthrottle_latency_ns, ~0 U);
