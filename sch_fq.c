...
int valuePresentInArray(unsigned val, unsigned arr[], int lengthOfarray) {
  ...
  for (i = 0; i < lengthOfarray; i++) {
    ...
    if (arr[i] == val) {
      printk(
          "In Array Present function match occured value present index value "
          "of %d\n",
          i);
      return i;
    }
...
Once a flow is identified to be part of a co-flow set and breach has occured ,
all the subsequent flows belnging to co-flow

set will be promoted to co-flow set, co-flows are searched in both ld and new
flow list
