...

{
    ...
    {   
        f = head->first;
   	if(head->first == flowmapper[0])
           {
	head->first = flowmapper[0];
	head->last = flowmapper[0];
           }
...
{      
                ...
                const char* pMap = bitmap;
              
		long int pVal;
                
                int pErr = kstrtol(pMap,5,&pVal);
               
                 kfree((char*)pMap);
                
                 printk("In dequeue pVal and pErr is : %ld --- %d \n ", pVal, pErr); 
*/
...
begin:      head = &q->new_flows;     
            ...
            if((head->first != flowmapper[0] ) && (head->first))
              {
                 while(head->first != flowmapper[0] )
                 {
                 printk ("match not found \n");
                 temp = head->first;
	...
	f = flowmapper[pCount_deq];
        printk("In the If loop of deleting flows : %d \n", f->credit );
        pCount_deq++;
	...
	if (!skb) {
		...
		if (((head == &q->co_flows) && q->old_flows.first)||((head == &q->new_flows) && q->old_flows.first)) {
			fq_flow_add_tail(&q->old_flows, f);
		} else {
			fq_flow_set_detached(f);
			q->inactive_flows++;
